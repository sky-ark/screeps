...
module.exports.loop = function () {
    ...
    const availableEnergy = spawn.energy + _.sum(extensions, 'energy');
    ...
    if (spawn.spawning) {
        ...
        } else if (availableEnergy >= 400) { // Adjust the energy threshold as needed
            // Spawn super creeps if there is enough energy
            if (harvesters.length < 6) {
                newName = 'SuperHarvester' + Game.time;
                const spawnResult = spawn.spawnCreep([WORK, WORK, CARRY, CARRY, MOVE, MOVE], newName, {
                    memory: {
                        role: 'harvester'
                    }
